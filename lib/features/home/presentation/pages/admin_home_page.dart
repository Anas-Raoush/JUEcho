...
///
/// Responsibilities:
/// - Displays welcome section derived from AuthProvider (cached profile name).
/// - Triggers AdminHomeStatsProvider.init() once to:
///   -> load initial stats
///   -> establish realtime listeners (GraphQL subscriptions)
/// - Renders high-level metrics via AdminStatsGrid.
/// - Provides navigation shortcuts to admin workflows.
///
/// Realtime:
/// - AdminHomeStatsProvider.init() must be called (not load()) to enable:
///   -> onCreateSubmission { id }
///   -> onUpdateSubmission { id }
///   -> onDeleteSubmission { id }
/// - Provider refreshes stats in response to any ping event.
